...
function refreshMetrics(couchdbClient) {
  couchdbClient.stats(function (err, stats) {
    ...
    // Mean Request Time
    var meanRequestTime = getStatVal(stats, 'couchdb.request_time.mean') + " ms";
    metrics.meanRequestTime.set(meanRequestTime);

    // Max Request Time
    var maxRequestTime = getStatVal(stats, 'couchdb.request_time.max') + " ms";
    metrics.maxRequestTime.set(maxRequestTime);
